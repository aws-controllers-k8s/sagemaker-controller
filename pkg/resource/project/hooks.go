...
var (
	modifyingStatuses = []string{
		string(svcsdktypes.ProjectStatusPending),
		string(svcsdktypes.ProjectStatusCreateInProgress),
		string(svcsdktypes.ProjectStatusUpdateInProgress),
		string(svcsdktypes.ProjectStatusDeleteInProgress),
	}
...
func (rm *resourceManager) customDescribeProjectSetOutput(ko *svcapitypes.Project) {
	svccommon.SetSyncedCondition(&resource{ko}, ko.Status.ProjectStatus, &resourceName, &modifyingStatuses)
}
