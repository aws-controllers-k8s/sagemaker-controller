...
// TestInferenceComponentTestSuite runs the test suite for InferenceComponent
func TestInferenceComponentTestSuite(t *testing.T) {
	defer func() {
		if r := recover(); r != nil {
...
func (d *testRunnerDelegate) EmptyServiceAPIOutput(apiName string) (interface{}, error) {
	...
	switch apiName {
	case "CreateInferenceComponentWithContext":
		var output svcsdk.CreateInferenceComponentOutput
		return &output, nil
	case "DeleteInferenceComponentWithContext":
		var output svcsdk.DeleteInferenceComponentOutput
		return &output, nil
	case "DescribeInferenceComponentWithContext":
		var output svcsdk.DescribeInferenceComponentOutput
		return &output, nil
	case "UpdateInferenceComponentWithContext":
		var output svcsdk.UpdateInferenceComponentOutput
		return &output, nil
	}
...
func (d *testRunnerDelegate) Equal(a acktypes.AWSResource, b acktypes.AWSResource) bool {
	...
	opts := []cmp.Option{
		...
		cmpopts.IgnoreFields(svcapitypes.InferenceComponentStatus{}, "CreationTime", "LastModifiedTime"),
