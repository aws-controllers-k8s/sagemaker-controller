...
func (rm *resourceManager) sdkFind(
	...
	rm.metrics.RecordAPICall("READ_ONE", "DescribeEndpointConfig", respErr)
	if respErr != nil {
		if awsErr, ok := ackerr.AWSError(respErr); ok && awsErr.Code() == "ValidationException" && strings.HasPrefix(awsErr.Message(), "Could not find endpoint configuration") {
			return nil, ackerr.NotFound
		}
		return nil, respErr
	}
...
func (rm *resourceManager) sdkCreate(
	...
	rm.metrics.RecordAPICall("CREATE", "CreateEndpointConfig", respErr)
	if respErr != nil {
		return nil, respErr
	}
	// Merge in the information we read from the API call above to the copy of
...
func (rm *resourceManager) sdkDelete(
	...
	rm.metrics.RecordAPICall("DELETE", "DeleteEndpointConfig", respErr)
	return respErr
}
