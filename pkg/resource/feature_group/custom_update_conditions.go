...
func (rm *resourceManager) CustomUpdateConditions(
	...
	koStatusConditions := &ko.Status.Conditions
	terminalMessage := "Feature group status reached terminal state: " + terminalStatus + ". Check the FailureReason."
	// If the latestStatus == terminalStatus we will set
	// the terminal condition and terminal message.
	return customShared.SetTerminalState(koStatusConditions, latestStatus, terminalStatus, &terminalMessage)
}
