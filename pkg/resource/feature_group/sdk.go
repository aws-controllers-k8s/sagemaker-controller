...
func (rm *resourceManager) sdkFind(
	...
	resp, respErr := rm.sdkapi.DescribeFeatureGroupWithContext(ctx, input)
	rm.metrics.RecordAPICall("READ_ONE", "DescribeFeatureGroup", respErr)
	if respErr != nil {
		if awsErr, ok := ackerr.AWSError(respErr); ok && awsErr.Code() == "ResourceNotFound" && strings.HasPrefix(awsErr.Message(), "Resource Not Found") {
			return nil, ackerr.NotFound
		}
		return nil, respErr
	}
...
func (rm *resourceManager) sdkCreate(
	...
	resp, respErr := rm.sdkapi.CreateFeatureGroupWithContext(ctx, input)
	rm.metrics.RecordAPICall("CREATE", "CreateFeatureGroup", respErr)
	if respErr != nil {
		return nil, respErr
	}
	// Merge in the information we read from the API call above to the copy of
...
func (rm *resourceManager) sdkDelete(
	...
	_, respErr := rm.sdkapi.DeleteFeatureGroupWithContext(ctx, input)
	rm.metrics.RecordAPICall("DELETE", "DeleteFeatureGroup", respErr)
	return respErr
}
