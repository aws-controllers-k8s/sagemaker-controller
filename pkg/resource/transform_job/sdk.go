...
func (rm *resourceManager) sdkFind(
	...
	rm.metrics.RecordAPICall("READ_ONE", "DescribeTransformJob", respErr)
	if respErr != nil {
		if awsErr, ok := ackerr.AWSError(respErr); ok && awsErr.Code() == "ValidationException" && strings.HasPrefix(awsErr.Message(), "Could not find requested job with name") {
			return nil, ackerr.NotFound
		}
		return nil, respErr
	}
...
func (rm *resourceManager) sdkCreate(
	...
	rm.metrics.RecordAPICall("CREATE", "CreateTransformJob", respErr)
	if respErr != nil {
		return nil, respErr
	}
	// Merge in the information we read from the API call above to the copy of
...
func (rm *resourceManager) sdkDelete(
	...
	_, respErr := rm.sdkapi.StopTransformJobWithContext(ctx, input)
	rm.metrics.RecordAPICall("DELETE", "StopTransformJob", respErr)
	return respErr
}
