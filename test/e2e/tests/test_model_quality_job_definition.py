...
"""Integration tests for the SageMaker ModelQualityJobDefinition API.
"""
...
def xgboost_churn_model_quality_job_definition(xgboost_churn_endpoint):
    endpoint_spec = xgboost_churn_endpoint
    endpoint_name = endpoint_spec["spec"].get("endpointName")

    job_definition_name = random_suffix_name("model-quality-job-definition", 32)
    ...
    reference, _, resource = create_sagemaker_resource(
        ...
        spec_file="model_quality_job_definition_xgboost_churn",
...
def get_sagemaker_model_quality_job_definition(sagemaker_client, job_definition_name):
    try:
        ...
        logging.error(
            f"Could not find Model Quality Job Definition with name {job_definition_name}. Error {error}"
        )
        return None
...
@service_marker
@pytest.mark.canary
class TestModelQualityJobDefinition:
    def test_smoke(self, sagemaker_client, xgboost_churn_model_quality_job_definition):
        (reference, resource) = xgboost_churn_model_quality_job_definition
        assert k8s.get_resource_exists(reference)

        job_definition_name = resource["spec"].get("jobDefinitionName")
        job_definition_desc = get_sagemaker_model_quality_job_definition(
